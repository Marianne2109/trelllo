...
#Comment is part of a card, therefore comment can't exist without a card
...
@comments_bp.route("/", methods=["POST"]) #dynamic route get from function
@jwt_required()
def create_comment(card_id):
    ...
    if card:
        ...
        comment = Comment(  #Class  follows Pascal case so starts with upper case
            message=body_data.get("message"),
            date=date.today(),
            card=card,
            user_id=get_jwt_identity()  #can pass a single id or the entire user model
    ...
    # else
        ...
        # return en error message like card does not exist
